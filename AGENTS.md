...
Performance benchmark work uses immutable versioned E0 baseline and accepted-calibration artifacts under `docs/benchmarks/`. The `reference` and `current` roles keep separate results; comparison may block only a repeated regression with matching workload and protocol whose measurements share both the exact environment fingerprint and one comparison-pair ID from a single runner session. A historical E0 measurement without that physical pairing remains diagnostic. For PR checks, the accepted E0 commit is read from the baseline, built in an isolated checkout, and measured beside head on the same runner; failure to measure it reports `not-measured` without masking the independent `base`/`head` verdict. PR checks use only a compatible accepted calibration; absence or runner/protocol mismatch reports `not-calibrated` without falling back to baseline tolerances. Full 20-pair calibration runs manually and on the weekly schedule and produces a candidate that requires explicit acceptance as a new immutable artifact. Calibration compares runner classes (Node.js, platform, architecture, runner image, and CPU count); independent jobs may have different CPU models, while both measurements inside one pair must use the exact environment. The PR gate must publish all simulation and Analyzer reports before enforcing a blocking verdict; the base adapter may bridge a missing numeric stage without changing base source. The gate receives the expected fresh-session pair ID and fails as an infrastructure error if a produced base, head, confirmation, or available E0 measurement loses that ID or disagrees on exact environment or protocol; this integrity failure does not reclassify legitimate diagnostic verdicts. Baseline replacement requires explicit calibration and epoch acceptance; the manual performance workflow produces the candidate E0 from matched CI reference and calibration artifacts.
