...
import { formatImportCompletenessReport, createImportCompletenessReport } from "../src/index.js";
...
test("import completeness report summarizes raw, draft, runtime, and draft validation gaps", () => {
  ...
  assert.match(output, /cards: raw 2, drafts 1 \(valid 1, invalid 0, warnings 0\), runtime 1/);
  assert.match(output, /wizard properties: raw 0, drafts 1 \(valid 0, invalid 1, warnings 0\), runtime 1/);
  assert.match(output, /dead wizard tokens: raw 1, drafts 1 \(valid 1, invalid 0, warnings 0\), runtime 0/);
  assert.match(output, /missing drafts: cards esw2_dbg__main_002/);
  assert.match(output, /missing runtime: dead wizard tokens esw2_dbg__dead_wizard_token_001/);
  assert.match(output, /runtime without valid draft: wizard properties esw2_dbg__wizard_property_002/);
...
test("import completeness report reads runtime cardId and nested tokenId files", () => {
  ...
  assert.match(output, /cards: raw 0, drafts 1 \(valid 1, invalid 0, warnings 0\), runtime 1/);
  assert.match(output, /wizard properties: raw 0, drafts 1 \(valid 1, invalid 0, warnings 0\), runtime 1/);
...
test("import completeness report keeps long gap lists concise", () => {
  ...
  assert.match(output, /missing drafts: cards esw2_dbg__main_001, esw2_dbg__main_002/);
