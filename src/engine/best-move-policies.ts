...
export const victoryPointsPolicy: TurnLineEvaluationPolicy = {
  ...
  evaluate: ({ line, perspectivePlayerId }) => {
    const player = scoreGame(line.terminalState).find((candidate) => candidate.playerId === perspectivePlayerId);
    if (player === undefined) throw new Error(`Missing perspective player ${perspectivePlayerId}`);
    return { score: player.victoryPoints, components: { victoryPoints: player.victoryPoints } };
  },
};

export const BEST_MOVE_POLICIES = { "victory-points": victoryPointsPolicy } as const;
...
export function getBestMovePolicy(id: string): TurnLineEvaluationPolicy {
  ...
  if (policy === undefined) throw new Error(`Unknown criterion "${id}". Available criteria: ${Object.keys(BEST_MOVE_POLICIES).join(", ")}`);
  return policy;
}
