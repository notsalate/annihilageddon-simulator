...
- `RunSingleGameOptions.botFactory` creates one `BotStrategy` for each `PlayerId`; strategy objects and their `chooseAction`/`chooseEffectChoice` callbacks must not serve multiple players. `bot: baselineBot` remains a deterministic compatibility path; other legacy `bot` values fail before setup and must migrate to `botFactory`.
