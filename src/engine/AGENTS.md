...
- `RunSingleGameOptions.botFactory` creates one `BotStrategy` for each `PlayerId`; one strategy instance must not serve multiple players. `bot` is a legacy single-instance option and fails when a second player would reuse it.
