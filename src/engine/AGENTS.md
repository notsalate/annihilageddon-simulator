...
- Benchmark workload fingerprints exclude the `reference`/`current` role; comparisons require the same epoch, workload volume, one warmup, three measurements, two players and a compatible accepted calibration. Workload changes, missing calibration and uncalibrated environments are non-blocking reports. A 20-pair run produces only a candidate; it cannot replace a versioned accepted calibration automatically.
