...
- `setup/auxiliary` owns setup, scoring and condition descriptors that are not bootstrap or Effective Value payloads; `effects/general` owns the remaining directly executed and explicitly unsupported effect payloads. Both families declare each ID's decoder, source-kind policy, runtime mode and, where the payload requires it, exact timing in the Catalog.
