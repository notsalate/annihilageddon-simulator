...
- `effective-values.ts` owns Effective Value discovery, stable modifier order, target matching, arithmetic, self-scoring, and specialized calculators; `effect-runtime-effective-value-modifier.ts` owns the modifier-family payloads, policy, decoder and handlers, while `effective-value-catalog.ts` exposes the applicability policy used by the runtime Catalog.
...
- CLI orchestration lives in `src/cli/`. `simulation.ts` owns structured failure reports containing setup parameters, runtime definitions, action/choice history, event-log context, error stacks and a reproduction command; `src/cli/simulation-menu.ts` only persists and formats those reports.
...
- Effect execution resolves raw or unverified effects through the legacy Catalog operation that decodes and invokes the concrete handler inside one typed closure. The transition typed path is limited to the resources/draw family: Runtime Data Intake supplies its verified payload, the typed Catalog checks source/mode/timing policy and dynamic applicability without a full decoder, and then invokes the family handler. Callers must not receive or reconnect a general handler/payload pair. Mayhem execution does not use a separate catalog lookup.
