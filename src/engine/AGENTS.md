...
- `benchmark-support.ts` owns benchmark clocks, environment fingerprints, commit identity and shared measurement helpers. `performance-epoch.ts` owns active-epoch baseline entries, versioned accepted calibration, 20-pair candidate calibration, workload comparison, verdicts and fresh PR report integrity; it must not silently adopt current results as reference or use baseline tolerances when no compatible accepted calibration exists.
...
- Public `applyAction` performs read-only preflight, returns expected refusals as `ok: false`, and turns any post-mutation refusal or exception into `ActionExecutionError` with deterministic action context. `attack-defense.ts` remains the nested sequential Defense savepoint.
- `physical-card-zone-snapshot.ts` owns the reusable physical-zone capture/restore used by the expected Defense branch rollback; Control Ledger owns the zone descriptors and ordinary card operations, not savepoint hooks.
...
- `deadWizardToken` may declare typed effective-value modifiers and scoring effects. Token scoring effects are evaluated only through the shared scoring path; they do not create a token-specific action dispatcher.
...
- Benchmark workload fingerprints exclude the `reference`/`current` role; comparisons require the same epoch, workload volume, one warmup, three measurements, two players and a compatible accepted calibration. Workload changes, missing calibration and uncalibrated environments are non-blocking reports. The PR gate separately validates the promised fresh-session pair, exact environment, protocol and confirmation before trusting those reports; metadata loss is an infrastructure failure, not a diagnostic verdict. A 20-pair run produces only a candidate; it cannot replace a versioned accepted calibration automatically.
