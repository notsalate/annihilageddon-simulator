...
- Attack Resolution uses focused modules with explicit ownership: `attack-resolution.ts` owns attack amount components, current-attacker modifiers and damage attribution; `attack-defense.ts` owns voluntary Defense, the single Defense-cost evaluator, immutable payment plans shared by legality and commit, payment/movement transaction, rollback and redirect chains. `effect-runtime.ts` remains the adapter and owns per-target lifecycle orchestration, attack events and final damage application; Mayhem and Mega Mayhem remain separate domain flows.
