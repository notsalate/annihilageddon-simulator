...
- `control-ledger.ts` также определяет источники добровольной Defense: обычную руку игрока и зарегистрированные extension-зоны. Перемещение такой карты между физическими зонами выполняется только через операции Ledger; участвующая extension-зона должна возвращать устойчивое изменяемое хранилище, пригодное для безусловного восстановления после ошибки `replace`.
...
- Trigger Dispatch exposes typed `onPlayCard`, `afterPlayerAttackDamage`, `afterDamageDealt` and `collectEndTurnDrawModifier` operations. The first three require an ongoing controlled card; end-turn discovery preserves temporary-control semantics and returns a typed aggregate instead of raw effects.
...
- `BotStrategy` receives an isolated active-player view and public legal actions only; each legal market purchase includes its effective open cost so baseline selection remains deterministic without reading `GameState`. Effect-choice views expose opaque candidate IDs, never candidate card definitions.
- `RunSingleGameOptions.botFactory` creates one `BotStrategy` for each `PlayerId`; strategy objects and their `chooseAction`/`chooseEffectChoice` callbacks must not serve multiple players. `bot: baselineBot` remains a deterministic compatibility path, and `botFactory` takes priority when both are present. Any other legacy `bot` value fails before setup, even when paired with `botFactory`, and must be removed in favor of the factory.
