...
- `run-benchmark.ts` keeps run, epoch comparison and calibration modes as thin wrappers over engine modules. `--output` paths are explicit machine-artifact destinations; comparison accepts only an explicitly versioned `--acceptedCalibration` artifact, while `--calibration` remains the input of calibration mode. CLI output must not rewrite the committed epoch baseline or accepted calibration.
