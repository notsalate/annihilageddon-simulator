...
The command defaults to the `reference` workload, which is eligible for E1 comparison only when its protocol and fingerprints match the accepted E1 baseline, including the exact environment fingerprint and one common non-empty `comparisonPairId`. A mismatch is reported as `incomparable`; a historical or standalone artifact without that physical pairing is `not-measured` and diagnostic-only. Pass `--role current` to inspect the current data set separately; its clean timing remains diagnostic and is not compared with E1.

The command runs four processes in order: the clean Analyzer benchmark, an instrumented semantic-counter run, a separate Node CPU-profile run, and a separate Node allocation-profile run. For a `reference` workload whose protocol, fingerprints, exact environment and pairing match the accepted E1 baseline, the clean benchmark is the only timing comparable with ADR-0001/E1; a mismatch is reported as `incomparable`, an unpaired run is `not-measured`, and the `current` workload has no E1 comparison. Instrumented and profiled timings are explicitly diagnostic-only and must not update the accepted baseline, performance epoch, or CI gate.

The summary contains the selected profile, workload/environment and result fingerprints, clean phase timings, semantic counters, physical-location counters, CPU category totals, sampled allocation totals, hotspots, and paths to the generated artifacts. Location counters cover point card-location searches, physical-zone passes, cards viewed, full location lists, location records, and cards whose zone or index changed between the start and end of an Analyzer branch attempt. The branch distribution reports searches performed inside each action attempt, including attempts interrupted by a choice request; searches used while listing legal actions remain in the total but are not assigned to a branch. Counters use array-item units for copied paths and event-log entries; non-overlapping phase counters separate enumeration, ranking, and evaluation-policy operations/isolation. All four runs must report the same result fingerprint.

CPU and allocation hotspots retain generated JavaScript URL, line, and column information. Allocation profiling uses a 64 KiB sampling interval and the human summary prioritizes project-code hotspots; the JSON artifact also retains the overall hotspots, including Node startup. For native/V8 allocation leaves, the allocation summary attributes the sample to the nearest JavaScript caller when one exists. Generated `dist/**/*.js.map` files provide the TypeScript source-mapping hint. Both profiles are sampled views: allocation bytes are not exact total allocation volume, and the allocation profile is not a heap snapshot. Location counters estimate repeated scan work and prospective index updates; they do not replace either profile.
