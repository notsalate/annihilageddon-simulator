...
Это целевой layout для новых изображений и согласованной миграции assets. Если существующее изображение еще лежит в исторической раскладке по видимому типу карты, `source.image` указывает на его фактический legacy-путь до отдельной миграции. Добавление runtime JSON само по себе не требует копировать или переносить изображение и не разрешает придумывать отсутствующий путь в целевой структуре. `special` используется для особых runtime-карт и стопок, например Wild Magic и Limp Wand. Слова `processed` в пути или данных не являются каноническим маркером статуса.
