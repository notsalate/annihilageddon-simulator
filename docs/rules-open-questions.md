...
Global runtime questions are resolved below or in the linked normative ADRs. Remaining blockers are data, mapping, policy, or component dependencies.
...
| RQ-001: proof of a non-terminating ATTACK chain                                                              | [Rules Canon](rules-canon.md) and [ADR-0009](adr/0009-attack-chain-cycle-proof.md) define the exact recurrence projection, `ChoicePolicy` snapshot requirement, typed `provenAttackChainCycle` outcome, and the rule that empty DWT or an opaque policy is not by itself a cycle proof. |
