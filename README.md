...
`baselineBot` — временная простая `BotStrategy`: он выбирает первый допустимый вариант и не оптимизирует линию. Будущие aggressive/defensive стратегии будут моделировать решения игрока с ограниченным наблюдением.

`Best-Move Analyzer` — отдельное пока не реализованное направление анализа. Он сможет перебирать полные линии текущего хода с доступом к complete engine state и воспроизводимым ветвлением seeded RNG; критерий оценки будет передаваться вызывающим кодом. Это не стратегия игрока и не обещание уже работающего анализатора.
